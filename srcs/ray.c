		...
		double *distance, t_vec3 *normal)
{
	...
	if (object->type == SPHERE)
		return (intersect_sphere(origin, dir, (t_sphere *)object->obj_data,
				distance, normal));
	if (object->type == PLANE)
		return (intersect_plane(origin, dir, (t_plane *)object->obj_data,
				distance, normal));
	if (object->type == CYLINDER)
		return (intersect_cylinder(origin, dir, (t_cylinder *)object->obj_data,
				distance, normal));
	return (0);
}
		...
		double light_distance)
{
	...
	while (object)
	{
		...
		if (object != hit->object && intersect_object(object, origin, light_dir,
				&distance, &normal) && distance < max_distance)
			return (1);
		object = object->next;
	}
	return (0);
